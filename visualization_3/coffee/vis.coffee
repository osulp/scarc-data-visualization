...
Network = () ->
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d["genre"] or d["Type"]))
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d["genre"] or d["Type"]))
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d["genre"] or d["Type"])).darker().toString()
  ...
  showDetails = (d,i) ->
    ...
    content += '<p class="main">Genre: ' + d["genre"] + '</span></p>' if d["genre"]?
...
$ ->
  ...
  d3.json "data/final_jane.json", (json) ->
    myNetwork("#vis", json)
